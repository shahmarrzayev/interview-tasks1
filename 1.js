...
function sostavChisla(massivChisel, chislo) {
  let result = [];
  function generateCombinations(array, combinationSize, currentIndex = 0, currentCombination = []) {;
    if (currentCombination.length === combinationSize) {
      let sum = currentCombination.reduce((a, b) => a + b, 0);
      if (sum == chislo) result.push([...currentCombination]);
      return;
    }
    if (currentIndex >= array.length) return;
    currentCombination.push(array[currentIndex]);
    generateCombinations(array, combinationSize, currentIndex + 1, currentCombination);
    currentCombination.pop();
    generateCombinations(array, combinationSize, currentIndex + 1, currentCombination);
  }
  
  for (let i = 1; i < massivChisel.length; i++){
    generateCombinations(massivChisel, i);
  }
	return result;
}
